...
The `ContextMenuService` provides a `closeAll` method that will close any open context menu. It also provides an `hasOpenMenu` to check if there is at leaset one.
